...
/* {
	...
	pieces.push_back(img);
	
	while(pieces.front().rows * pieces.front().cols > 60 * 60)
	{
	...
	{
		char name[20]="";
		sprintf(name, "test_out_%d.png", i);
	...
	while (pieces.size() > 1)
	{
		...
		sprintf(name, "test_out_merge%d.png", j++);
		
		for (i=0; i<4 && i<pieces.size(); i++){
			printf("copy.. %d/%llu\n", i+1, pieces.size()); 
			quarter[i] = pieces.at(i);
		}
		printf("erase.. from %d / %llu\n", i, pieces.size()); 
		pieces.erase(pieces.begin(), pieces.begin()+i);
		
		printf("vec size: %llu\n", pieces.size()); 
		
		cv::Mat merged;
		
		printf("created merged mat..\n"); 
	...
	cv::imwrite("test_out.png", pieces.front());
	return 0;
}
